...
def home():
    ...
    return render_template("home.html", user=utils.get_user(), products=products, expires=session['pass_expr'],
...
def login():
    if request.method == "POST":
        user = utils.email_exists(request.form.get("email", ""))
        if not user:
            return 'Account does not exist', 404
        msg, code = utils.login_attempts or [None, None]
        if msg:
            return msg, int(code)
        else:
